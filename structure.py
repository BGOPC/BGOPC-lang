...
class Lexer:
        ...
        def make_not_equals(self):
                ...
                if self.current_char == '=':
                        self.advance()
                        return Token(NEqual, pos_start=pos_start, pos_end=self.pos), None

                self.advance()
                return None, ExpectedCharError(pos_start, self.pos, "'=' was expected after '!'")
...
class Parser:
        ...
        def statement(self):
                ...
                if self.current_tok.matches(KeyWord, 'continue'):
                        res.register_advancement()
                        self.advance()
                        return res.success(ContinueNode(pos_start, self.current_tok.pos_start.copy()))

                if self.current_tok.matches(KeyWord, 'break'):
                        res.register_advancement()
                        self.advance()
        ...
        def expr(self):
                ...
                node = res.register(self.bin_op(
                        self.comp_expr, ((KeyWord, 'and'), (KeyWord, 'or'))))
        ...
        def comp_expr(self):
                ...
                node = res.register(self.bin_op(
                        self.arith_expr, (EEqual, NEqual, Less, Greater, LE, GE)))
        ...
        def atom(self):
                ...
                elif tok.matches(KeyWord, 'function'):
        ...
        def for_expr(self):
                ...
                if self.current_tok.matches(KeyWord, 'stp'):
                        res.register_advancement()
                        self.advance()
        ...
        def func_def(self):
                ...
                if not self.current_tok.matches(KeyWord, 'function'):
                        return res.failure(InvalidSyntaxError(
                                self.current_tok.pos_start, self.current_tok.pos_end,
                                f"Expected 'function'"
                        ))
...
class Interpreter:
        ...
        def visit_BinOpNode(self, node, context):
                ...
                elif node.op_tok.type == EEqual:
                        result, error = left.get_comparison_eq(right)
                elif node.op_tok.type == NEqual:
                        result, error = left.get_comparison_ne(right)
                elif node.op_tok.type == Less:
                        result, error = left.get_comparison_lt(right)
                elif node.op_tok.type == Greater:
                        result, error = left.get_comparison_gt(right)
                elif node.op_tok.type == LE:
                        result, error = left.get_comparison_lte(right)
                elif node.op_tok.type == GE:
...
global_symbol_table.set("IfFunc", BuiltInFunction.is_function)
